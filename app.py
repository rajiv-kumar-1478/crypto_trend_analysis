...
def get_sentiment_data():
    ...
    daily_sentiment['date'] = daily_sentiment['date'].dt.strftime('%b %d, %Y')  # Include year

    daily_price_change = filtered.groupby('date')['close_price_next_day_minus_same_day'].mean().reset_index()
    daily_price_change['date'] = daily_price_change['date'].dt.strftime('%b %d, %Y')  # Include year
    ...
    counts['date'] = pd.to_datetime(counts['date']).dt.strftime('%b %d, %Y')  # Include year
